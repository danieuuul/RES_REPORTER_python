...
class Parser:
    ...
    def np_updated(self, OILVOLSC_series, DATE_series, date, i):
        temp = pd.concat([DATE_series, OILVOLSC_series], axis = 1)        
        return ((temp['OILVOLSC'] - self.shift(temp['OILVOLSC'], 1))/(1 + i)**(temp['DATE'].dt.year - date.year)).cumsum()

    def shift(self, series, shift):
        return series.shift(shift).fillna(0)
    
    def derivative(self, series, offset):
        return (series - self.shift(series, 1))/(offset - self.shift(offset, 1))
            
    def instantValue(self, VAR_series, DATE_series, date):
        temp = pd.concat([DATE_series, VAR_series], axis = 1)     
        return temp.loc[temp['DATE'] == date].head(1).iloc[-1][1]
    ...
    def __init__(self):
        ...
        self.functions = {
            ...
            'atan2': math.atan2,
            'concat':self.concat,
            'np_updated': self.np_updated,
            'shift': self.shift,
            'derivative': self.derivative,
            'instantValue': self.instantValue,
            'if': self.ifFunction
        }
